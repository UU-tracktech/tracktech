...
import detection.dectection_obj as DetectionObject
import detection.bounding_box as BoundingBox
import tracking.tracking_obj as TrackingObject
...
def main(_argv):
    ...
    det_obj = DetectionObject.DetectionObj(t, None, 0)
...
def draw_and_show(frame, boxes):
    ...
    for i in boxes:
        ...
        cv2.rectangle(frame, (int(i.rectangle[0]), int(i.rectangle[1])), (int(i.rectangle[2]), int(i.rectangle[3])), color, 2)
        cv2.rectangle(frame, (int(i.rectangle[0]), int(i.rectangle[1] - 30)),
                      (int(i.rectangle[0]) + (len(i.classification) + len(str(i.identifier))) * 17, int(i.rectangle[1])), color, -1)
        cv2.putText(frame, i.classification + "-" + str(i.identifier), (int(i.rectangle[0]), int(i.rectangle[1] - 10)), 0, 0.75,
                    (255, 255, 255), 2)
    ...
    #This next line is **ESSENTIAL** for the video to actually play I am not sure why, but it is mostly the waitKey command
