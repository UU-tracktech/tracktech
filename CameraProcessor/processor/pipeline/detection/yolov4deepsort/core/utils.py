...
from processor.pipeline.detection.yolov4deepsort.core.config import cfg
...
def load_config(framework):
    if framework=='yolov4-tiny':
        STRIDES = np.array(cfg.YOLO.STRIDES_TINY)
        ANCHORS = get_anchors(cfg.YOLO.ANCHORS_TINY, True)
        XYSCALE = cfg.YOLO.XYSCALE_TINY if framework == 'yolov4' else [1, 1]
    else:
        STRIDES = np.array(cfg.YOLO.STRIDES)
        if framework == 'yolov4':
            ANCHORS = get_anchors(cfg.YOLO.ANCHORS, False)
        elif framework == 'yolov3':
            ANCHORS = get_anchors(cfg.YOLO.ANCHORS_V3, False)
        XYSCALE = cfg.YOLO.XYSCALE if framework == 'yolov4' else [1, 1, 1]
