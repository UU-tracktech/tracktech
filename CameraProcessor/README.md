...
These feature maps are send to the orchestrator which in turn shares it with other camera proccessor.
This means that all camera processors must use the same re-identification algorithm, 
and once a new algorithm is chosen all processor, and the orchestrator must be restarted.

- Restart orchestrator once re-identification algorithm is switched.
- Use same re-identification algorithm across all processors.
